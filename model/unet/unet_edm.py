...
class SongUNet(torch.nn.Module):
    ...
    def forward(self, x, node_flags, noise_labels, class_labels=None, augment_labels=None):
        # Tensor shape trimming.
        if len(x.shape) == 3:
            x = x.unsqueeze(1)  # [B, 1, N, N] <- [B, N, N]
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
                 ...
                 self_condition=False  # self_conditioning
                 ):
        super().__init__()
        self.label_dropout = label_dropout
        emb_channels = model_channels * channel_mult_emb
        ...
        if self_condition:
            first_block_in_channels = in_channels * 2
        else:
    ...
    def forward(self, x, node_flags, noise_labels, self_cond=None, class_labels=None, augment_labels=None):
